...
function getMetaData(id) {
  ...
  d3.json(sampleData).then(function (data) {
    ...
    var metadataFiltered = metadata_array.filter(
...
function bonus(id) {
  ...
  d3.json(sampleData).then(function (data) {
    ...
    var gaugedataFiltered = gaugedata.filter(
      (gauge) => gauge.id.toString() === id
    )[0];
...
function createPlots(id) {
  ...
  d3.json(sampleData).then(function (data) {
    ...
    var plotsdataFiltered = plotsData.filter(
      (chart) => chart.id.toString() === id
    )[0];
    ...
    var trace = {
      ...
      text: otu_labelstop,
      marker: { size: sample_valuestop, color: "teal" },
      orientation: "h",
    };
    var data = [trace];
    var layout = {
...
function init() {
  ...
  d3.json(sampleData).then(function (data) {
    ...
    sampleNames.forEach((sampleNames) => {
      selOptions
        .append("option")
        .text(sampleNames)
